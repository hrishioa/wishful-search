...
// prettier-ignore
const MARKDOWN_TEMPLATE = (inputObj: any, typespec: string, ddlExplanation: string, ddl: string, structuredDDL: string, objectToRow: string) =>
...
export async function autoAnalyzeObject(
  ...
  let typespec = await callLLM(messages);
  ...
  let tableStructure = await callLLM(messages);

  if (!tableStructure)
    throw new Error('No response from LLM while generating table structure!');

  console.log('\n####################\nGot table structure - ', tableStructure);
  ...
  let ddl = await callLLM(messages);
  ...
  let structuredDDL = await callLLM(structuredDDLMessages);
  ...
  console.log('\n#################\nGot structured DDL - ', structuredDDL);
  ...
  let objectToRowFunc = await callLLM(objectToRowMessages);
  ...
  console.log('\n#################\nGot objectToRow - ', objectToRowFunc);
